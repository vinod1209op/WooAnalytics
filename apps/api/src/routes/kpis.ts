...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const { endExclusive, prevFrom, prevEndExclusive } = buildComparisonRanges(
      fromDate,
      toDate
    );

    const resolvedType = typeof type === "string" ? type : "date";
    const { whereOrders, prevWhereOrders, leadRateWhere, prevLeadRateWhere } =
      buildOrderWheres({
        storeId,
        fromDate,
        endExclusive,
